    ...
    'backup' => [
        ...
        'source' => [
            'files' => [
                 ...
                 */
                'include' => [
                    // Private uploads only (surat keterangan scans, santri
                    // photos, etc). App code lives in git, not the backup -
                    // base_path() would otherwise sweep in vendor/, node_modules/
                    // and this very backups disk on every run.
         ...
         *
         * Left null deliberately: GzipCompressor shells out to an external
         * `gzip` binary, which isn't reliably on PATH on Windows (unlike
         * mysqldump/mysql, there's no equivalent DB_DUMP_BINARY_PATH escape
         * hatch for it in db-dumper). The zip archive itself is still
         * compressed (see destination.compression_method/level below), so
         * dump size still shrinks - just in one layer instead of two.
         */
         ...
         */
        // 'connection' keeps the dump filename tied to the connection name
        // ("mysql"), which BackupService relies on when locating the dump
        // to restore - the database *name* is a config value that could
        // change, the connection name in source.databases above won't.
