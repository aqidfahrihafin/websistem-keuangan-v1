...
use App\Http\Controllers\Api\Wali\TabunganController;
...
Route::prefix('wali')->name('api.wali.')->group(function () {
    ...
    Route::post('/quick-login', [AuthController::class, 'quickLogin'])->name('quick-login')->middleware('throttle:6,1');
    // Public - drives the mobile app's splash/login branding, which render
    // before any session exists.
    Route::get('/app-info', AppInfoController::class)->name('app-info')->middleware('throttle:public-api');
    // Public - Home tab's banner carousel, site-wide content rather than
    // per-wali data, so it doesn't need auth either.
    Route::get('/banners', BannerController::class)->name('banners')->middleware('throttle:public-api');

    Route::middleware(['auth:sanctum', 'ability:wali', 'throttle:api'])->group(function () {
        ...
        Route::post('/password', [AuthController::class, 'password'])->name('password')->middleware('throttle:auth-sensitive');

        Route::get('/pin/status', [PinController::class, 'status'])->name('pin.status');
        Route::post('/pin/confirm-password', [PinController::class, 'confirmPassword'])->name('pin.confirm-password')->middleware('throttle:auth-sensitive');
        Route::post('/pin', [PinController::class, 'store'])->name('pin.store')->middleware('throttle:auth-sensitive');
        ...
        Route::get('/anak/{santri}/tabungan', [TabunganController::class, 'show'])->name('anak.tabungan');
        Route::post('/anak/{santri}/tabungan/dari-saldo', [TabunganController::class, 'dariSaldo'])->name('anak.tabungan.dari-saldo')->middleware('throttle:financial');
        Route::post('/anak/{santri}/tabungan/midtrans', [TabunganController::class, 'viaMidtrans'])->name('anak.tabungan.midtrans')->middleware('throttle:financial');
        ...
        Route::post('/anak/{santri}/tagihan/{tagihan}/bayar', [TagihanController::class, 'bayar'])->name('anak.tagihan.bayar')->middleware('throttle:financial');
        Route::post('/anak/{santri}/tagihan/{tagihan}/topup/core', [TagihanController::class, 'bayarViaMidtrans'])->name('anak.tagihan.topup.store-core')->middleware('throttle:financial');
        Route::post('/anak/{santri}/topup', [TopupController::class, 'store'])->name('anak.topup.store')->middleware('throttle:financial');
        Route::post('/anak/{santri}/topup/core', [TopupController::class, 'storeCore'])->name('anak.topup.store-core')->middleware('throttle:financial');
        ...
        Route::post('/topup/{topup}/sync', [TopupController::class, 'sync'])->name('topup.sync')->middleware('throttle:payment-sync');

        Route::get('/unit-usaha/{kode}', [UnitUsahaController::class, 'show'])->name('unit-usaha.show');
        Route::post('/anak/{santri}/bayar-kantin', [UnitUsahaController::class, 'bayar'])->name('anak.bayar-kantin')->middleware('throttle:financial');

        Route::get('/anak/{santri}/saudara', [SaudaraController::class, 'index'])->name('anak.saudara');
        Route::post('/anak/{santri}/transfer', [TransferController::class, 'transfer'])->name('anak.transfer')->middleware('throttle:financial');
