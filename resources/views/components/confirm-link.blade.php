...
<span x-data="{ confirmOpen: false }" class="inline-block">
    ...
    >
        ...
        <div
            ...
            class="relative w-full max-w-sm rounded-2xl border border-white/80 bg-white/95 p-6 shadow-2xl ring-1 ring-slate-900/10 backdrop-blur-xl"
        >
            ...
            <div class="mt-6 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
                <button type="button" x-on:click="confirmOpen = false" class="btn-secondary">{{ $cancelText }}</button>
                <a href="{{ $href }}" @if ($target) target="{{ $target }}" @endif x-on:click="confirmOpen = false" class="{{ $confirmClass }}">{{ $confirmText }}</a>
            </div>
        </div>
    </div>
</span>
