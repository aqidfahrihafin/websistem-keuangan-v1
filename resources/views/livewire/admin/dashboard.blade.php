...
<div class="space-y-5">
    ...
    <div class="admin-control-welcome relative overflow-hidden rounded-md bg-gradient-to-r from-slate-950 via-teal-900 to-teal-700 p-5 text-white shadow-lg sm:p-6">
        ...
        <svg class="dashboard-hero-art pointer-events-none absolute inset-y-0 right-0 h-full w-2/3 text-white/10" viewBox="0 0 500 140" fill="currentColor" preserveAspectRatio="xMaxYMax slice" aria-hidden="true">
        ...
        <div class="relative flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
            <div class="flex min-w-0 items-center gap-4">
                ...
                <div class="min-w-0">
                    <p class="text-xs font-semibold uppercase tracking-[.12em] text-teal-700">{{ now()->translatedFormat('l, d F Y') }}</p>
                    <h1 class="truncate text-xl font-bold leading-tight sm:text-2xl">Halo, {{ \Illuminate\Support\Str::before(auth()->user()->name, ' ') }}.</h1>
                    <p class="truncate text-sm text-slate-600">Berikut kondisi terbaru {{ $nama_aplikasi }}.</p>
                </div>
            </div>
    ...
    <div class="dashboard-panel grid overflow-hidden sm:grid-cols-2 xl:grid-cols-4">
        ...
        @foreach ($perluPerhatian as $item)
            <a href="{{ $item['href'] }}" class="group flex h-24 min-w-0 items-center gap-3 border-b border-slate-100 p-4 transition hover:bg-slate-50 sm:odd:border-r xl:border-b-0 xl:border-r xl:last:border-r-0">
                <span class="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl {{ $item['icon_class'] }}">
                ...
                <span class="min-w-0 flex-1">
                    <span class="block text-xl font-bold tabular-nums {{ $item['value'] > 0 ? $item['value_class'] : 'text-slate-900' }}">{{ number_format($item['value']) }}</span>
                    <span class="block truncate text-xs font-medium text-slate-600">{{ $item['label'] }}</span>
                </span>
    ...
    {{-- Kartu ringkasan tanpa judul section. --}}
    <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <a href="{{ route('admin.santri.index') }}" class="card flex h-24 min-w-0 items-center gap-3 p-4 transition hover:-translate-y-0.5">
            ...
            <span class="min-w-0">
                <span class="block truncate text-sm text-slate-500">Santri Aktif</span>
                <span class="block truncate text-2xl font-bold text-slate-900 tabular-nums">{{ number_format($santri_aktif) }}</span>
            </span>
        </a>
        <div class="card flex h-24 min-w-0 items-center gap-3 p-4">
            ...
            <span class="min-w-0">
                <span class="block truncate text-sm text-slate-500">Total Saldo Santri</span>
                <span class="block truncate text-xl font-bold text-slate-900 tabular-nums">Rp {{ number_format($saldo_santri_total, 0, ',', '.') }}</span>
            </span>
        </div>
        <a href="{{ route('admin.leger-kas-pondok.index') }}" class="card flex h-24 min-w-0 items-center gap-3 p-4 transition hover:-translate-y-0.5">
            ...
            <span class="min-w-0">
                <span class="block truncate text-sm text-slate-500">Saldo Kas Pondok</span>
                <span class="block truncate text-xl font-bold text-emerald-700 tabular-nums">Rp {{ number_format($saldo_kas_pondok, 0, ',', '.') }}</span>
            </span>
        </a>
        <a href="{{ route('admin.tagihan.index') }}" class="card flex h-24 min-w-0 items-center gap-3 p-4 transition hover:-translate-y-0.5">
            ...
            <span class="min-w-0">
                <span class="block truncate text-sm text-slate-500">Tagihan Belum Lunas</span>
                <span class="block truncate text-2xl font-bold text-slate-900 tabular-nums">{{ number_format($tagihan_belum_lunas) }}</span>
            </span>
        </a>
    </div>
    ...
    <div class="grid grid-cols-1 items-stretch gap-4 sm:grid-cols-2">
        <div class="card flex h-80 flex-col overflow-hidden p-4 transition hover:shadow-md">
            <p class="text-[10px] font-bold uppercase tracking-[.12em] text-teal-700">Arus transaksi</p>
            <h2 class="mt-1 truncate text-base font-bold text-slate-900">Tren Transaksi (30 Hari Terakhir)</h2>
        ...
        <div class="card flex h-80 flex-col overflow-hidden p-4 transition hover:shadow-md">
            <p class="text-[10px] font-bold uppercase tracking-[.12em] text-teal-700">Posisi kas</p>
            <h2 class="mt-1 truncate text-base font-bold text-slate-900">Saldo Kas Pondok (30 Hari Terakhir)</h2>
        ...
        <div class="card flex h-80 flex-col overflow-hidden p-4 transition hover:shadow-md">
            <p class="text-[10px] font-bold uppercase tracking-[.12em] text-teal-700">Komposisi</p>
            <h2 class="mt-1 truncate text-base font-bold text-slate-900">Status Tagihan</h2>
        ...
        <div class="card flex h-80 flex-col overflow-hidden p-4 transition hover:shadow-md">
            <p class="text-[10px] font-bold uppercase tracking-[.12em] text-teal-700">Data santri</p>
            <h2 class="mt-1 truncate text-base font-bold text-slate-900">Status Santri</h2>
