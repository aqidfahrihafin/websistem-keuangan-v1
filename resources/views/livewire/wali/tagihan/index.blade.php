    ...
    @else
        ...
        <div class="table-card">
            <table class="min-w-full divide-y divide-slate-200 text-sm">
                <caption class="sr-only">Daftar tagihan {{ $santri->nama }}</caption>
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse ($tagihans as $tagihan)
                        <tr wire:key="tagihan-{{ $tagihan->id }}">
                            ...
                            <td class="px-4 py-3">
                                <x-status-badge :tone="$statusTone[$tagihan->status] ?? 'slate'">{{ ucwords(str_replace('_', ' ', $tagihan->status)) }}</x-status-badge>
                            </td>
                            <td class="px-4 py-3 text-right">
                                @if (in_array($tagihan->status, ['belum_lunas', 'sebagian']))
                                    <a href="{{ route('wali.tagihan.bayar', $tagihan) }}" class="btn-link">Bayar dari Saldo</a>
                        ...
                        <tr>
                            <td colspan="6" class="p-4">
                                <x-empty-state
                                    ...
                                    :description="filled($search) || filled($status) ? 'Coba ubah kata kunci atau filter status.' : 'Tagihan santri akan tampil di sini setelah diterbitkan.'"
                                />
                            </td>
                        </tr>
                    @endforelse
